module AppFusedEffects where

import Commons (embeddedLog)
import qualified Data.Map as Map
import Internal.FusedEffects
import Types

type Eval a =
  ErrorC String (ReaderC Env (WriterC Variables (StateC Steps (LiftC IO)))) a

runEval :: Env -> Steps -> Eval Value -> IO (Either String Value, Steps, Variables)
runEval env steps ev =
  (\(steps', (variables, result)) -> (result, steps', variables))
    <$> runM (runState steps (runWriter (runReader env (runError ev))))

tick :: Has (State Steps) sig m => m ()
tick = do
  st <- get
  put (st + 1)
...
eval ::
  ( Has (Error String) sig m,
    ...
    Has (State Steps) sig m,
    Has (Writer Variables) sig m,
    Has (Lift IO) sig m,
    MonadIO m
  ) =>
  Exp ->
