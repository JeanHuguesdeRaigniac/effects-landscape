...
If you want to have a look at and run this code on your computer, open a terminal and type `cabal run effects-landscape`. Otherwise, here is the output from Martin Grabmüller's code. Beside the first line, other libraries output is identical.
...
Each library version has 2 modules. One in [`Internal`](/app/Internal/) to hide specific imports and align bits of code to match our reference code. We name the other according to the `App<LibraryName>.hs` format. Besides imports, it contains only 4 things:
...
This same format for each `App<LibraryName>.hs` module results in a very uniform and boring [`Main.hs`](/app/Main.hs) module. We call all libraries in the exact same way, the expected signature of a benchmark.
...
Since we are considering library usage, implementations in [`Internal`](/app/Internal/) folder are not analyzed.
...
Take [`AppFreerSimple.hs`](/app/AppFreerSimple.hs) for example. If you switch back to the library version of `get` and try to compile, you will have this error:
...
The first one is polymorphic, it comes from the library. The second one is monomorphic, this is the one we export from the [`FreerSimple.hs`](/app/Internal/FreerSimple.hs) module. It makes explicit `State`'s parameter type.
...
The [`Inference.hs`](/app/Inference.hs) module gives various ways to fix the problem. I use`freer-simple` for illustration purpose. All libraries of its kind suffer from the same issue.
...
All you have to do is plug this library into [`effects-landscape.cabal`](effects-landscape.cabal). Then, write the [`Internal`](/app/Internal/)`/<OtherLibrary>.hs` and `App<OtherLibrary>.hs` modules. Call the latter in [`Main.hs`](/app/Main.hs) and run it to check its behavior.
...
Of course, if you know the reason and the fix, send a PR. All you need to provide is 1) a section in this file and 2) a module following the example given in the [`Inference.hs`](/app/Inference.hs) module.
